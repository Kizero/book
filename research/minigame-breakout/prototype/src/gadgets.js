import {updateExtras,evolved,availableEvolutions} from './combinations.js?v=11';
...
export class Gadgets {
 ...
 reset(){this.bubbles=[];this.arcs=[];this.pops=[];this.discs=[];this.buckets=[];this.timer={brush:.6,bucket:.3,bubble:.4,electric:.8,duck:.5};this.phase=0;this.duck={x:this.g.player.x-38,y:this.g.player.y+28};this.serial=0;this.washCooldown=0;this.deliveryCooldown=0;}
 ...
 commandDelivery(){
  const g=this.g;if(!evolved(g,'stormduck')||this.deliveryCooldown>0)return;
  const ready=this.bubbles.filter(b=>!b.popped&&dist(this.duck,b)<160).slice(0,3);if(!ready.length)return;
  this.deliveryCooldown=.6;
  for(const b of ready){b.charged=true;b.command=2.5;b.vx=Math.cos(g.player.angle)*220;b.vy=Math.sin(g.player.angle)*220;}
  g.stats.deliveries=(g.stats.deliveries||0)+1;g.evolutionUse('stormduck');g.emit('delivery',{x:this.duck.x,y:this.duck.y});
 }
 triggerWash(shot){
  const g=this.g;if(!evolved(g,'washstorm')||this.washCooldown>0||shot.washTriggered||!g.foam.some(f=>dist(f,shot)<f.r))return;
  shot.washTriggered=true;this.washCooldown=1.2;this.discharge({x:shot.x,y:shot.y});g.evolutionUse('washstorm');g.stats.manualDischarges=(g.stats.manualDischarges||0)+1;g.emit('wash-trigger',{x:shot.x,y:shot.y});
 }
 discharge(start=null){
  this.pops.push({x:this.g.player.x,y:this.g.player.y,r:115,life:.4});
  const g=this.g,origins=[start||g.player],visited=new Set(),targets=new Set();if(!start&&g.mods.duck)origins.push(this.duck);
  for(let i=0;i<origins.length;i++){
   const origin=origins[i],range=i<(!start&&g.mods.duck?2:1)?115:origin.r+12;
 ...
 update(dt){
  const g=this.g,p=g.player;this.phase+=dt*3.5;this.washCooldown=Math.max(0,this.washCooldown-dt);this.deliveryCooldown=Math.max(0,this.deliveryCooldown-dt);
  ...
  for(const b of this.bubbles){
   ...
   let vx=b.vx,vy=b.vy;b.command=Math.max(0,(b.command||0)-dt);
   if(b.command>0){}else if(g.mods.duck&&dist(this.duck,b)<100){if(evolved(g,'stormduck'))b.charged=true;const target=g.enemies.find(e=>e.alive&&!e.trapped);if(target){const d=dist(target,b)||1;vx=(target.x-b.x)/d*80;vy=(target.y-b.y)/d*80;}}
