import {renderRunRecap} from './recap.js?v=10';
import {createGameKeyboard} from './keyboard.js?v=10';
import {upgradeFeedback} from './upgrade-feedback.js?v=10';
import {COMMISSIONS,loadJournal,rememberRoom} from './commissions.js?v=10';
import {CleaningSound} from './sound.js?v=10';
import {PRESETS,EVOLUTIONS} from './combinations.js?v=10';
import {GADGETS,activeSynergies} from './gadgets.js?v=10';
import {Game,UPGRADES,ROOM_NAMES,ROOMS,WEAPONS,CLEAN_TARGET} from './engine.js?v=10';
...
function syncOverlay(){
  ...
  if(game.state==='upgrade'){
    overlay.hidden=false;overlay.innerHTML=`<div class="modal upgrade-modal"><span class="modal-eyebrow">ROOM ${String(game.room).padStart(2,'0')} · ${game.clearReason==='clean'?'清洁达标 · 怪物撤退':'捣蛋鬼已清空'}</span><h2>干得漂亮，升级一下。</h2><p>体力恢复一格。选一件新装备，去下一个院子。</p><div class="menu-keys">点击选择 · 键盘 Tab 切换，Enter 确认</div><div class="upgrades">${game.offers.map(id=>UPGRADES.find(u=>u.id===id)).map(u=>`<button class="upgrade" data-upgrade="${u.id}"><img class="upgrade-icon" src="assets/original/${u.icon}.svg" alt=""/><span class="category">${u.category}</span><strong>${u.name}${GADGETS.includes(u.id)?` · ${['I','II','III'][game.mods[u.id]]}`:''}</strong><small>${u.description}</small>${upgradeFeedback(game,u.id)?`<span class="pairing">${upgradeFeedback(game,u.id)}</span>`:''}</button>`).join('')}</div></div>`;
    const cards=overlay.innerHTML;
    ...
    $('show-upgrades').onclick=()=>{overlay.classList.remove('showcase-overlay');overlay.innerHTML=cards;overlay.querySelectorAll('[data-upgrade]').forEach(b=>b.onclick=()=>{game.chooseUpgrade(b.dataset.upgrade);tone(640,.16);syncOverlay();hud();});overlay.querySelector('button').focus({preventScroll:true});};
    $('show-upgrades').focus({preventScroll:true});
...
function frame(now){
  ...
  for(const e of game.drainEvents()){
    ...
    if(e.type==='evolution')toast('✦ 连携成型：'+e.name);
